...
export H2_URI="jdbc:mysql://129.194.69.129:13306/ACADEMI_CO_DB"
...
# Add the datasource
data-source add --name=Academi-co_DS --driver-name=mysql --jndi-name=$ACADEMI_CO_DS --connection-url=$MYSQL_URI?useUnicode=true&amp;characterEncoding=UTF-8 --user-name=$MYSQL_USER --password=$MYSQL_PWD --use-ccm=false --max-pool-size=25 --blocking-timeout-wait-millis=5000
...
/subsystem=security/security-domain=academi-co-realm/authentication=classic                                         \
   :add(login-modules=[                                                                                      \
           {"code"=>"Database", "flag"=>"required",                                                          \
            "module-options"=>{"dsJndiName" => "$ACADEMI_CO_DS",                                   \
